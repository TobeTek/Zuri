...
class Budget(
    Category
):  # Budget class inherits Category methods to be able to manipulate categories
    ...
    def transfer(self, name, target, amt):
        if name in self.names:
            self.categories[self.names.index(name)].transferAmount(
                self.categories[self.names.index(target)], amt
            )
        else:
